...
// Mock the server-side chart loader to prevent it from executing its real backend code
jest.mock('@/app/(app)/admin/dashboard/assetChartLoader', () => {
  return function MockAssetChartLoader({ entityView }: any) {
    return <div data-testid="mock-chart-loader">Chart Data: {entityView}</div>
  }
})
...
// Create a mock resolved searchParams promise defaulting to 'assets'
const mockSearchParams = Promise.resolve({ entityView: 'assets' })
...
describe('DashboardPage', () => {
  ...
  // A shared variable to store the component across render cycles
  ...
  beforeEach(() => {
    ...
    jest.mocked(useAdminAccess).mockReturnValue({
      session: mockSession,
      isLoading: false,
    } as any); // Type assertion to satisfy TypeScript
  ...
  const renderDashboard = async () => {
    dashboardElement = await DashboardPage({ searchParams: mockSearchParams })
  ...
  it('shows loading screen while session is loading', async () => {
    ...
    dashboardElement = await DashboardPage({ searchParams: mockSearchParams })
    await act(async () => { render(dashboardElement) })
  ...
  it('updates entityView when dropdown selection changes to "department"', async () => {
    ...
    // Assert that the component successfully tried to navigate to the department view parameter
    expect(mockPush).toHaveBeenCalledWith(expect.stringContaining('entityView=department'))
  })
  ...
  it('updates entityView when dropdown selection changes to "location"', async () => {
    ...
    // Assert that the component successfully tried to navigate to the location view parameter
    expect(mockPush).toHaveBeenCalledWith(expect.stringContaining('entityView=location'))
  })

  it('renders the RealtimeChart with correct initial entityView prop', async () => {
    await renderDashboard()
    await waitFor(() => {
      // Main layout utilizes the mocked chart-loader, not realtime-chart directly (WC)
      expect(screen.getByTestId('mock-chart-loader')).toHaveTextContent('assets')
    })
  })
