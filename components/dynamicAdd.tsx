...
// Database column name - used as the key in formData and the POST body
// Human-readable label shown above the input
// Determines which HTML element is rendered for this field
// Shows whether the field is required or not
...
// Configuration object passed to DynamicAdd from each entity's add page
// entityName: Short internal name for logging, e.g., 'asset'
// entityDisplayName: Plural display name, e.g., 'Assets'
// entityDisplayNameSingular: Singular display name, e.g., 'Asset'
// apiEndpoint: API route to POST the form data, like /api/assets
// primaryKey: Primary key field name, e.g., 'asset_id'
// formFields: Ordered list of form fields to render
// pageTitle: Page heading shown at the top
// backUrl: URL to redirect to on save or cancel
...
// It renders the form based on the config passed in, and handles form submission,
// input changes, duplicate checking and related data loading
export default function DynamicAdd({ config }: dynamicAddProps) {
  ...
  // Handle the input changes - copy everything from previous object, then update the changed field
  ...
  // Handle form submission - prevent the user from submitting if validation fails or duplicate is taken
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If no location_id or if it's empty, set to null
      if (!submissionData['location_id'] || submissionData['location_id'] === '') { 
        submissionData['location_id'] = null 
      }
      // If no department_id or if it's empty, set to null
  ...
  // Handles text inputs, text areas, select dropdowns, and numbers
  ...
  // hasClientErrors - client-side validation failed (regex, maxLength, etc.)
