...
async function withRetry<T>(fn: () => Promise<T>, retries = 3, baseDelayMs = 2000): Promise<T> {
  ...
  throw new Error('Gemini retry limit reached'); // unreachable but satisfies TS
}

// ─── Step 1: Pre-screen the image before full assessment ──────────────────────
// This first call is cheap (small prompt) and acts as a gatekeeper.
// If the image is not a recognised asset, we reject early and save quota.
async function identifyAsset(imageBase64: string, mimeType: string): Promise<{
...
// ─── Step 2: Asset-specific assessment criteria ───────────────────────────────
// Each asset type has its own tailored checklist so the model knows exactly
// what defects to look for, improving accuracy significantly.
function getAssetCriteria(assetType: string): string {
...
- General: heavy dirt/debris buildup, missing feet/stands`,
  ...
  // Fuzzy match — e.g. "office chair" → Chair criteria
...
export async function assessFurnitureCondition(
  ...
  try {
    // ── Gate 1: Identify the asset ──────────────────────────────────────────
    console.log('Step 1: Identifying asset type...');
    const identification = await withRetry(() => identifyAsset(imageBase64, mimeType));
    console.log('Identification result:', identification);

    if (!identification.isValidAsset) {
      // Reject non-asset images cleanly — no assessment is saved
      throw new Error(
    ...
    // ── Gate 2: Asset-specific condition assessment ─────────────────────────
    const assetCriteria = getAssetCriteria(identification.detectedAsset);
...
4. Battery swelling on any electronic = always Spoiled + High`;
    ...
    // Surface INVALID_ASSET errors cleanly to the API route
...
// ─── Response parser (unchanged logic, added robustness) ─────────────────────
