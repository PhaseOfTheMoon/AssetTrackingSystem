...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const parsed = rejectStaffSchema.safeParse(body)
    if (!parsed.success) {
    ...
    if (error) {
      ...
      console.error('Error rejecting staff:', { message: error.message })
      return NextResponse.json(
    ...
    console.error('Reject staff error:', { message: error?.message })
    return NextResponse.json(
